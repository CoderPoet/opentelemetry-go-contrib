...
func (p *producerMessageContext) recordMetric(ctx context.Context, cfg config, requestStartTime time.Time) {
	// Use floating point division here for higher precision (instead of Millisecond method).
	elapsedTime := float64(time.Since(requestStartTime)) / float64(time.Millisecond)
	// record request counter
	cfg.instruments.requestsCounter.Add(ctx, 1, p.attrs...)
	// record request latency
	cfg.instruments.requestLatencyHistogram.Record(ctx, elapsedTime, p.attrs...)
}
